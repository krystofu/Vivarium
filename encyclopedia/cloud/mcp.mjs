...
export async function handleMcp(request,env,store,auth) {
  ...
  tool('fetch','Fetch a canonical Vivarium character','Read the authoritative Vivarium Gen 2 character record before editing. Returns every approved Identity Reference and marks the primary default. Use get_identity_reference_for_generation to place one exact original into the conversation for image generation.',{id:z.string().max(100)},false,async({id})=>{
    ...
    const identityReferences=assets.filter(a=>a.kind==='identity'&&a.status==='Accepted'&&a.locked).map(a=>({...a,canonicalPrimary:a.id===c.primaryAssetId}));
    return {id,title:c.name,url:`${env.SITE_ORIGIN}/characters/${id}`,character:c,assets,identityReferences,primaryIdentity:primary,canonicalImageAvailable:!!(primary?.locked&&primary.status==='Accepted')};
  });
  ...
  tool('list_identity_references','List approved identity references','List every locked Accepted image in a character’s Identity References tab. The primary image is marked as the default, but any listed assetId may be selected for generation.',{characterId:z.string().max(100)},false,async({characterId})=>{
    ...
    const identityReferences=db.assets.filter(a=>a.characterId===characterId&&a.kind==='identity'&&a.status==='Accepted'&&a.locked).map(a=>({...publicAsset(a),canonicalPrimary:a.id===character.primaryAssetId}));
    return {characterId,characterName:character.name,primaryAssetId:character.primaryAssetId,identityReferences};
  });
  tool('get_identity_reference_for_generation','Get an identity reference for image generation','Return the exact stored original for image generation as an MCP image content block. Provide assetId to select any image shown in Identity References; omit it to use the canonical primary. After this succeeds, use this returned image directly as the image-generation reference (the most recent conversation image); do not ask for or invent a local filesystem path.',{characterId:z.string().max(100),assetId:z.string().max(100).optional()},false,async({characterId,assetId})=>{
    const db=await store.read(),character=db.characters.find(c=>c.id===characterId);if(!character)throw Object.assign(new Error('Character not found.'),{status:404});
    const selectedId=assetId||character.primaryAssetId,asset=db.assets.find(a=>a.id===selectedId);
    ...
    const found=await originalAsset(env,store,asset.id),metadata={asset:publicAsset(asset),characterId,characterName:character.name,canonicalPrimary:asset.id===character.primaryAssetId,sha256:asset.sha256,generationReady:true,referenceHandoff:'Use the returned MCP image content block as the image-generation reference.'};
